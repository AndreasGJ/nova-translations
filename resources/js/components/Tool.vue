...
const walkThroughTree = (tree = {}, group, reg, selected_locale) => {
    ...
    for (let key in tree) {
        const x = tree[key];
        const { new: newValue, old, lastValue } = x;
        ...
        if (
            ...
            (!reg ||
                (reg &&
                    (full_key.match(reg) ||
                        (newValue && newValue.match(reg)) ||
                        (lastValue && lastValue.match(reg)) ||
                        old.match(reg))))
        ) {
            list.push({
                ...x,
    ...
    computed: {
        filtered_list() {
            ...
            if (this.tree && selected_locale) {
                const search_term = (this.search_term || "").replace(
                    /\./g,
                    "\\."
                );
        ...
        saveTranslation(trans) {
            ...
            Nova.request()
                ...
                .then(({ data: rsp = {} }) => {
                    ...
                    this.search_term = full_key;

                    this.$toasted.show("Translation is saved!", {
                        type: "success"
                    });
                })
                .catch(error => {
